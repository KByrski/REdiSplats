...
__constant__ LaunchParams<SH_degree> optixLaunchParams;
...
__device__ void __raygen__() {
	...
	int pixel_ind = (y * optixLaunchParams<SH_degree>.width) + x;

	REAL3_R d = make_REAL3_R(
		(((REAL_R)-0.5) + __fdividef(x + ((REAL_R)0.5), optixLaunchParams<SH_degree>.width)) * optixLaunchParams<SH_degree>.double_tan_half_fov_x,
		(((REAL_R)-0.5) + __fdividef(y + ((REAL_R)0.5), optixLaunchParams<SH_degree>.height)) * optixLaunchParams<SH_degree>.double_tan_half_fov_y,
		1
	);
	REAL3_R v = make_REAL3_R(
		MAD_R(optixLaunchParams<SH_degree>.R.x, d.x, MAD_R(optixLaunchParams<SH_degree>.D.x, d.y, optixLaunchParams<SH_degree>.F.x * d.z)),
		MAD_R(optixLaunchParams<SH_degree>.R.y, d.x, MAD_R(optixLaunchParams<SH_degree>.D.y, d.y, optixLaunchParams<SH_degree>.F.y * d.z)),
		MAD_R(optixLaunchParams<SH_degree>.R.z, d.x, MAD_R(optixLaunchParams<SH_degree>.D.z, d.y, optixLaunchParams<SH_degree>.F.z * d.z))
	);
	...
	do {
		...
		optixTrace(
			optixLaunchParams<SH_degree>.traversable,
			optixLaunchParams<SH_degree>.O,
			v,
			tMin, // !!! !!! !!!
			INFINITY,
			0.0f,
		...
		for (i = 0; i < rp.Gauss_num; ++i) {
			...
			float4 GC_1 = optixLaunchParams<SH_degree>.GC_part_1[ind];
			float4 GC_2 = optixLaunchParams<SH_degree>.GC_part_2[ind];
			float4 GC_3 = optixLaunchParams<SH_degree>.GC_part_3[ind];
			float2 GC_4 = optixLaunchParams<SH_degree>.GC_part_4[ind];
			...
			float3 O = make_float3(
				optixLaunchParams<SH_degree>.O.x - GC_2.x,
				optixLaunchParams<SH_degree>.O.y - GC_2.y,
				optixLaunchParams<SH_degree>.O.z - GC_2.z
			);
			...
			if constexpr (SH_degree >= 1) {
				float4 GC_2 = optixLaunchParams<SH_degree>.GC_part_2[ind];
				float4 GC_SH_1 = optixLaunchParams<SH_degree>.GC_SH_1[ind];
				float4 GC_SH_2 = optixLaunchParams<SH_degree>.GC_SH_2[ind];
				...
				if constexpr (SH_degree >= 2) {
					float4 GC_SH_3 = optixLaunchParams<SH_degree>.GC_SH_3[ind];
					float4 GC_SH_4 = optixLaunchParams<SH_degree>.GC_SH_4[ind];
					float4 GC_SH_5 = optixLaunchParams<SH_degree>.GC_SH_5[ind];
					float4 GC_SH_6 = optixLaunchParams<SH_degree>.GC_SH_6[ind];
					...
					if constexpr (SH_degree >= 3) {
						float4 GC_SH_7 = optixLaunchParams<SH_degree>.GC_SH_7[ind];
						float4 GC_SH_8 = optixLaunchParams<SH_degree>.GC_SH_8[ind];
						float4 GC_SH_9 = optixLaunchParams<SH_degree>.GC_SH_9[ind];
						float4 GC_SH_10 = optixLaunchParams<SH_degree>.GC_SH_10[ind];
						float4 GC_SH_11 = optixLaunchParams<SH_degree>.GC_SH_11[ind];
						...
						if constexpr (SH_degree >= 4) {
							float4 GC_SH_12 = optixLaunchParams<SH_degree>.GC_SH_12[ind];
							float4 GC_SH_13 = optixLaunchParams<SH_degree>.GC_SH_13[ind];
							float4 GC_SH_14 = optixLaunchParams<SH_degree>.GC_SH_14[ind];
							float4 GC_SH_15 = optixLaunchParams<SH_degree>.GC_SH_15[ind];
							float4 GC_SH_16 = optixLaunchParams<SH_degree>.GC_SH_16[ind];
							float4 GC_SH_17 = optixLaunchParams<SH_degree>.GC_SH_17[ind];
							float4 GC_SH_18 = optixLaunchParams<SH_degree>.GC_SH_18[ind];
							...
							float GC_SH_12 = optixLaunchParams<SH_degree>.GC_SH_12[ind];
							BSH = MAD_R(tmp, GC_SH_12, BSH);
						}
					}
				} else {
					float GC_SH_3 = optixLaunchParams<SH_degree>.GC_SH_3[ind];
			...
			if (!optixLaunchParams<SH_degree>.inference) {
				if (RSH_clamped == RSH) ind |= 536870912;
				if (GSH_clamped == GSH) ind |= 1073741824;
				if (BSH_clamped == BSH) ind |= 2147483648;

				optixLaunchParams<SH_degree>.Gaussians_indices[(number_of_Gaussians * optixLaunchParams<SH_degree>.width * optixLaunchParams<SH_degree>.height) + pixel_ind] = ind;
			...
			++number_of_Gaussians;
			condition = (number_of_Gaussians == optixLaunchParams<SH_degree>.max_Gaussians_per_ray) ||	(T < optixLaunchParams<SH_degree>.ray_termination_T_threshold);
			if (condition)
				break;
		}
	...
	R = MAD_R(T, optixLaunchParams<SH_degree>.bg_color_R, R);
	G = MAD_R(T, optixLaunchParams<SH_degree>.bg_color_G, G);
	B = MAD_R(T, optixLaunchParams<SH_degree>.bg_color_B, B);
	...
	optixLaunchParams<SH_degree>.bitmap[pixel_ind] = (Ri << 16) + (Gi << 8) + Bi;
	...
	if (optixLaunchParams<SH_degree>.inference)
		...
		if (number_of_Gaussians < optixLaunchParams<SH_degree>.max_Gaussians_per_ray)
			optixLaunchParams<SH_degree>.Gaussians_indices[(number_of_Gaussians * optixLaunchParams<SH_degree>.width * optixLaunchParams<SH_degree>.height) + pixel_ind] = -1;

		// *** *** *** *** ***

		optixLaunchParams<SH_degree>.bitmap_out_R[(y * (optixLaunchParams<SH_degree>.width + 11 - 1)) + x] = R_clamped; // !!! !!! !!!
		optixLaunchParams<SH_degree>.bitmap_out_G[(y * (optixLaunchParams<SH_degree>.width + 11 - 1)) + x] = G_clamped; // !!! !!! !!!
		optixLaunchParams<SH_degree>.bitmap_out_B[(y * (optixLaunchParams<SH_degree>.width + 11 - 1)) + x] = B_clamped; // !!! !!! !!!

		// *** *** *** *** ***

		max_RSH = ((optixLaunchParams<SH_degree>.bg_color_R > max_RSH) ? optixLaunchParams<SH_degree>.bg_color_R : max_RSH);
		max_GSH = ((optixLaunchParams<SH_degree>.bg_color_G > max_GSH) ? optixLaunchParams<SH_degree>.bg_color_G : max_GSH);
		max_BSH = ((optixLaunchParams<SH_degree>.bg_color_B > max_BSH) ? optixLaunchParams<SH_degree>.bg_color_B : max_BSH);
		...
		optixLaunchParams<SH_degree>.max_RSH[pixel_ind] = max_RSH; // !!! !!! !!!
		optixLaunchParams<SH_degree>.max_GSH[pixel_ind] = max_GSH; // !!! !!! !!!
		optixLaunchParams<SH_degree>.max_BSH[pixel_ind] = max_BSH; // !!! !!! !!!
	}
}
